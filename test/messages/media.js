...
describe('Menssagens-usuario-midia', function () {
    ...
    it('Deve avisar que não têm essa mensagem quando ela for encaminhada e não estiver no banco de dados', function (done) {
        ...
        client.forwardMessages(process.env.TEST_WAID, process.env.TEST_MEDIA_MSG_ID, true);
    });
    ...
    it('Deve adicionar a mensagem no banco de dados', function (done) {
            ...
            async function check_db(messages){
                try{
                    old_msg = await client.getMessageById(process.env.TEST_MEDIA_MSG_ID);
                    var content = await wa.decryptMedia(old_msg);       
                    var media_md5 = md5(content);
